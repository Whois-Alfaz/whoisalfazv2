...
async function run() {
  try {
    const featured = await uploadImage('manychat_n8n_featured', 'manychat_n8n_featured.webp');
    const body1 = await uploadImage('manychat_n8n_body1', 'manychat_n8n_body1.webp');
    const body2 = await uploadImage('manychat_n8n_body2', 'manychat_n8n_body2.webp');
    ...
    // Merge new metadata
    existingMeta.manychat_n8n_featured = featured;
    existingMeta.manychat_n8n_body1 = body1;
    existingMeta.manychat_n8n_body2 = body2;

    fs.writeFileSync(outputMetaPath, JSON.stringify(existingMeta, null, 2), 'utf-8');
    console.log(`💾 Day 4 image assets merged and saved successfully to: ${outputMetaPath}`);
  } catch (error) {
