...
date: "2026-03-09"
seoTitle: "Automated Marketing Reporting: Build Weekly Lead Summaries in n8n | Day 14"
...
Unlike the [Facebook Lead Capture pipeline we built in Day 13](/blog/automated-facebook-leads-n8n), which processes a single JSON payload at a time, today's architecture must process an array of JSON payloads. It must fetch 50 rows of data, group them by a specific attribute (like UTM Source), count them, and then output a single, consolidated summary.
...
- **The Database:** A structured ledger containing your leads. For this tutorial, we will use the [Google Sheets database we established in Day 8](/blog/capture-n8n-lead-data). Ensure your sheet has a Timestamp column and a Source column.
